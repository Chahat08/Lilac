...
ColorRGB pointColor(GLint x, GLint y) {
    ColorRGB color;
    glReadPixels(x + 250, HEIGHT + y - 290, 1, 1, GL_RGB, GL_FLOAT, &color);
    return color;
}
//bool hibyeblah = true;
...
void bresenham_line(Shape& l)
{
    ...
    if (!plot.empty())
    {
        ...
        for (int i = 0; i < pts.size(); ++i)
        {
            if (plot[i])
            {
                if (pts[i].first < -250 || pts[i].first > 250 || pts[i].second < -250 || pts[i].second>250) continue;
                glBegin(GL_POINTS);
...
int main()
{
    ...
    glLoadIdentity();
    glOrtho(-WIDTH / 2, WIDTH / 2, -WIDTH / 2, WIDTH / 2 + 40, 0.0, 1.0);
    ...
    int start_x = (-WIDTH / 2) + 5, start_y = (HEIGHT / 2) + 15;// btns are 30 px tall and wide, 5 px spacing bw them.
